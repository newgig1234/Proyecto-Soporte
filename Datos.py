    ...
    def validarUsuario(self, leg, psw):
        ...
        try:
            ...
            val = conn.query(Alumno).filter(
                Alumno.legajo == leg and Alumno.password == psw).fetchone()
    ...
    def infoMat(self, cm):
        ...
        try:
            ...
            nm = conn.query(ComisionMateria.materia.nombreMateria).filter(
                ComisionMateria.materia_id == cm.materia_id).fetchone()
            nc = conn.query(ComisionMateria.comision.nroComision).filter(
                ComisionMateria.comision_id == cm.comision_id).fetchone()
        except SQLAlchemyError as e:
            error = str(e.__dict__['orig'])
            print(error)
        finally:
...
class AlumnoComisionMateria(Base):  # clase
    ...
    def bajaMateria(self, a, cm):
        try:
            conn = crearConexion()
            conn.query(AlumnoComisionMateria).filter(AlumnoComisionMateria.alumno.id ==
                                                     a.id and AlumnoComisionMateria.comisionMateria.comision_materia_id == cm.comision_materia_id).delete()
            conn.commit()
        except SQLAlchemyError as e:
            error = str(e.__dict__['orig'])
            print(error)
