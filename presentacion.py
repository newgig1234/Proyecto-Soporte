...
class Exito(Popup):
    ...
    def construir(self, msj):
        mensaje = Label(text=msj)
        btn = Button(text="Aceptar")
        # se puede modificar para simplificar
        btn.bind(on_release=lambda x: self.cerrar())
...
class MostrarInformacion(Popup):
    ...
    def darBajaMaeteria(self, mc, u):
        ...
        if amc:
            ex = Exito(
                'Se elimino la materia de su lista correctamente.',
                title='Exito',
                size_hint=(
                    None,
                    None),
            ...
            er = Error(
                'No se pudo eliminar la materia de su lista, por favor intente nuevamente.',
                title='Error',
                size_hint=(
                    None,
                    None),
...
class loginPopup(Popup):
    ...
    def build(self):
        ...
        btn.bind(
            on_release=lambda x: self.controlLogin(
                txtIUser.text,
                txtIPsw.text))  # funcion para logear
    ...
    def controlLogin(self, leg: str, psw: str):
        if (leg != '' and psw != ''):
            ...
            er = Error(
                'Complete todos los campos', title='Error', size_hint=(
                    None, None), size=(
                    600, 200))
            er.open()
...
class registroPop(Popup):
    ...
    def build(self):
        ...
        btnAceptar.bind(
            on_release=lambda x: self.validarUsuario(
                txtILegajo.text,
                txtINombre.text,
                txtIApellido.text,
                txtIMail.text,
    ...
    def validarUsuario(self, leg, nom, app, mail, cont1, cont2):
        if (4 < len(leg) < 6 and 5 < len(cont1) and 5 < len(cont2)):
            if '' not in (leg, nom, app, mail, cont1, cont2) and ('@' in mail and '.' in mail):
                if(cont1 == cont2):
                    a = Alumno()
                    # instanciar     #registrar ususario en tabal usuarios
                    a.altaUsuario(leg, nom, app, mail, cont1)
                    ex = Exito('Se registro el usuario con exito',
                         title='Exito', size_hint=(None, None), size=(600, 200))
                    ex.open()
                    ...
                    er = Error(
                        'Las contrasenas son diferentes', title='Error', size_hint=(
                            None, None), size=(
                            600, 200))
                    er.open()
                    reg = registroPop()
                ...
                er1 = Error(
                    'Complete todos los campos de manera correcta', title='Error', size_hint=(
                        None, None), size=(
                        600, 200))
                er1.open()
        else:
            er2 = Error(
                'Legajo y/o Password no tiene la longitud necesaria',
                title='Error',
                size_hint=(
                    None,
                    None),
...
class SeleccionarMateria(Popup):
    ...
    def build(self, u):
        ...
        for button in buttons:
            button.bind(
                on_release=lambda button: self.selecCom(
                    button.text, u))
...
class SeleccionarComision(Popup):
    ...
    def build(self, nombre_materia, u):
        ...
        buttons = [Button(text=comision.nroComision)
                   for comision in comisiones]
        for button in buttons:
            button.bind(
                on_release=lambda button: self.confirmar(
                    nombre_materia, button.text, u))
            gLayout.add_widget(button)
        box.add_widget(txt)
        box.add_widget(gLayout)
    ...
    def confirmar(self, nombre_materia, numero_comision, u):
        box = BoxLayout(orientation='vertical')
        txt = Label(
            text='¿Desea inscribirse a {materia} en la comision {comision}?'.format(
                materia=nombre_materia,
                comision=numero_comision))
        gLayout = GridLayout(cols=2)
        btnCancelar = Button(text='Cancelar')
        btnCancelar.bind(
            on_release=lambda x: self.build(
                nombre_materia,
                u))  # funcion boton cancelar
        btnAceptar = Button(text='Aceptar')
        btnAceptar.bind(
            on_release=lambda x: self.altaACM(
                nombre_materia,
                numero_comision,
                u))  # funcion boton aceptar
        box.add_widget(txt)
    ...
    def altaACM(self, nombre_materia, numero_comision, u):
        ...
        if res:
            ex = Exito(
                '¡Se agrego la materia con exito!', title='Exito', size_hint=(
                    None, None), size=(
                    600, 200))
            ex.open()
            mp = MenuPrincipal()
            ...
            er = Error(
                'No se pudo agregar la materia, intente nuevamente desde el comienzo.',
                title='Error',
                size_hint=(
                    None,
                    None),
